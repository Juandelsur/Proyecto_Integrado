...
// ✅ CORRECCIÓN: Recibir el ID como prop desde la ruta (igual que movimientos)
 ...
 * ✅ CORRECCIÓN: Usar props.id directamente y hacer GET directo
 */
async function cargarActivo() {
  try {
    // Convertir el ID de string a número
    const activoId = parseInt(props.id)
    console.log('🔍 Cargando prop:', (parseInt(props.id)))
    console.log('🔍 Cargando activo con ID:', activoId)
    
    // LLAMADA DIRECTA AL ENDPOINT ESPECÍFICO (igual que movimientos)
    ...
    formulario.value = {
      tipo: response.data.tipo?.id || null,
      marca: response.data.marca,
      modelo: response.data.modelo,
      numero_serie: response.data.numero_serie || '',
      estado: response.data.estado?.id || null,
      ubicacion_actual: response.data.ubicacion_actual?.id || null,
      notas: response.data.notas || ''
    }
    
    console.log('✅ Activo cargado:', activo.value)
...
async function guardar() {
  ...
  try {
    // ✅ CORRECCIÓN: Usar activo.value.id que ya tenemos cargado
