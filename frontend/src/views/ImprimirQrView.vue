  ...
  <div class="print-labels-view">
    ...
    <v-card class="mb-4">
      ...
      <v-window v-model="activeTab">
        ...
        <!-- TAB 4: QR UBICACIONES -->
        <v-window-item value="ubicaciones-qr">
          <v-card-text>
            <v-alert type="info" variant="tonal" class="mb-4">
              <v-icon start>mdi-information</v-icon>
              Selecciona las ubicaciones para generar sus códigos QR. Cada ubicación tendrá su etiqueta con código QR basado en su código único.
            </v-alert>
            ...
            <v-row class="mb-4">
              <v-col cols="12" md="6">
                <v-select
                  v-model="filtros.departamentoQR"
                  :items="departamentos"
                  item-title="nombre_departamento"
                  item-value="id"
              ...
              <v-col cols="12" md="6">
                <v-text-field
                  v-model="filtros.busquedaUbicacionQR"
                  label="Buscar ubicación"
                  prepend-inner-icon="mdi-magnify"
                  variant="outlined"
                  density="comfortable"
                  clearable
                  hint="Buscar por nombre o código de ubicación"
                  persistent-hint
                ></v-text-field>
              </v-col>
            </v-row>
            ...
            <!-- Resumen y Selección -->
            <v-card variant="tonal" color="purple" class="mb-4" v-if="ubicacionesFiltradasQR.length > 0">
              <v-card-text>
                <v-row align="center">
                  <v-col cols="12" md="8">
                    <div class="d-flex align-center gap-3">
                      <v-chip color="purple" variant="elevated">
                        {{ ubicacionesQRSeleccionadas.length }} de {{ ubicacionesFiltradasQR.length }} seleccionadas
                      </v-chip>
                      
                      <v-checkbox
                        v-model="seleccionarTodasUbicacionesQR"
                        label="Seleccionar todas"
                        hide-details
                        @update:model-value="toggleSeleccionarTodasUbicacionesQR"
                      ></v-checkbox>
                    </div>
                  </v-col>
                </v-row>
            ...
            <v-card v-if="ubicacionesFiltradasQR.length > 0">
              <v-card-title class="text-h6 font-weight-bold d-flex justify-space-between">
                <span>Ubicaciones Disponibles</span>
                <span class="text-subtitle-2 text-grey">{{ ubicacionesFiltradasQR.length }} ubicaciones</span>
              </v-card-title>
              ...
              <v-list>
                <v-list-item
                  v-for="ubicacion in ubicacionesFiltradasQR"
                  :key="ubicacion.id"
                  :value="ubicacion.id"
                >
                  <template v-slot:prepend>
                    <v-checkbox
                      :model-value="isUbicacionQRSeleccionada(ubicacion.id)"
                      @update:model-value="toggleUbicacionQR(ubicacion)"
                      hide-details
                    ></v-checkbox>
                  </template>
                  ...
                  <v-list-item-subtitle>
                    <div class="d-flex align-center gap-2 flex-wrap mt-1">
                      <v-chip size="small" variant="tonal" color="purple" v-if="ubicacion.codigo_qr">
                        <v-icon start size="small">mdi-qrcode</v-icon>
                        {{ ubicacion.codigo_qr }}
                      </v-chip>
                      <v-chip size="small" variant="tonal" color="info" v-if="ubicacion.departamento">
                        {{ ubicacion.departamento.nombre_departamento }}
                      </v-chip>
                    </div>
                  </v-list-item-subtitle>
                </v-list-item>
            ...
            <v-card v-else class="text-center py-8">
              <v-card-text>
                ...
                <p class="text-body-2 text-grey">Intenta ajustar los filtros</p>
              </v-card-text>
            </v-card>

            <v-card-actions class="mt-4" v-if="ubicacionesQRSeleccionadas.length > 0">
              <v-spacer></v-spacer>
              <v-btn
                color="purple"
                size="large"
                @click="agregarUbicacionesQRACola"
              >
                <v-icon start>mdi-plus-circle</v-icon>
                Agregar {{ ubicacionesQRSeleccionadas.length }} Ubicaciones a Cola
              </v-btn>
            </v-card-actions>
          </v-card-text>
        </v-window-item>
    ...
    <!-- COLA DE IMPRESIÓN -->
    <v-card v-if="colaImpresion.length > 0 || colaUbicacionesQR.length > 0" class="mb-4">
      ...
      <v-card-text>
        <!-- Activos en Cola -->
        <div v-if="colaImpresion.length > 0" class="mb-4">
          <div class="text-subtitle-2 font-weight-bold mb-2">
            Activos ({{ colaImpresion.length }})
          </div>
          <v-chip-group column>
            <v-chip
              v-for="activo in colaImpresion"
              :key="'activo-' + activo.id"
              closable
        ...
        <!-- Ubicaciones QR en Cola -->
        <div v-if="colaUbicacionesQR.length > 0">
          <div class="text-subtitle-2 font-weight-bold mb-2">
            Ubicaciones ({{ colaUbicacionesQR.length }})
          </div>
          <v-chip-group column>
            <v-chip
              v-for="ubicacion in colaUbicacionesQR"
              :key="'ubicacion-' + ubicacion.id"
              closable
              @click:close="removerUbicacionQRDeCola(ubicacion.id)"
              color="purple"
              variant="outlined"
            >
              <v-icon start size="small">mdi-map-marker</v-icon>
              {{ ubicacion.codigo_qr || ubicacion.nombre_ubicacion }}
            </v-chip>
          </v-chip-group>
        </div>
      </v-card-text>
    ...
    <v-btn
      v-if="colaImpresion.length > 0 || colaUbicacionesQR.length > 0"
      class="fab-preview"
      color="success"
      size="x-large"
      elevation="8"
    ...
    >
      ...
      Vista Previa / Imprimir ({{ colaImpresion.length + colaUbicacionesQR.length }})
    </v-btn>
    ...
    >
      <v-card>
        <v-toolbar color="primary" dark>
          ...
          <v-toolbar-title>Vista Previa de Etiquetas ({{ colaImpresion.length + colaUbicacionesQR.length }} elementos)</v-toolbar-title>
          <v-spacer></v-spacer>
        ...
        <v-card-text class="pa-4">
          ...
          <div id="print-area" class="print-container">
            ...
            >
              <!-- Contenido de la Etiqueta -->
              <div class="etiqueta-content">
                <!-- Izquierda: Nombre de la Ubicación (70%) -->
                <div class="etiqueta-nombre">
                  <span class="nombre-text">{{ ubicacion.nombre_ubicacion }}</span>
                </div>
                ...
                <div class="etiqueta-qr-section">
                  ...
                  <div class="qr-container">
                    <img
                      v-if="ubicacion.qrDataUrl"
                      :src="ubicacion.qrDataUrl"
                      alt="QR Code Ubicación"
                      class="qr-image"
                    />
                    <div v-else class="qr-placeholder">
                      <v-icon size="40">mdi-qrcode</v-icon>
                  ...
                  <!-- Código Vertical -->
                  <div class="codigo-vertical">
                    <span>{{ ubicacion.codigo_qr || 'SIN-CODIGO' }}</span>
                  </div>
                </div>
              </div>
            </div>
 ...
 * Cuatro modos de selección: Por Activos, Por Ubicaciones, Manual, QR Ubicaciones.
...
const colaUbicacionesQR = ref([])
const ubicacionesQRSeleccionadas = ref([])

const seleccionarTodosActivos = ref(false)
const seleccionarTodosUbicacion = ref(false)
const seleccionarTodasUbicacionesQR = ref(false)
...
const ubicacionesFiltradasQR = computed(() => {
  let resultado = ubicaciones.value

  // Filtrar por departamento
  if (filtros.value.departamentoQR) {
    resultado = resultado.filter(ub => ub.departamento?.id === filtros.value.departamentoQR)
  }

  // Filtrar por búsqueda de texto
  if (filtros.value.busquedaUbicacionQR) {
    const termino = filtros.value.busquedaUbicacionQR.toLowerCase()
    resultado = resultado.filter(ub => {
      return (
        ub.nombre_ubicacion?.toLowerCase().includes(termino) ||
        ub.codigo_qr?.toLowerCase().includes(termino)
      )
    })
  }
...
function limpiarTodasLasColas() {
  colaImpresion.value = []
  colaUbicacionesQR.value = []
  mostrarNotificacion('Todas las colas limpiadas', 'info')
}
...
// ============================================================================
// MÉTODOS - SELECCIÓN DE UBICACIONES QR
// ============================================================================

function isUbicacionQRSeleccionada(ubicacionId) {
  return ubicacionesQRSeleccionadas.value.some(ub => ub.id === ubicacionId)
}

function toggleUbicacionQR(ubicacion) {
  const index = ubicacionesQRSeleccionadas.value.findIndex(ub => ub.id === ubicacion.id)
  
  if (index > -1) {
    ubicacionesQRSeleccionadas.value.splice(index, 1)
  } else {
    ubicacionesQRSeleccionadas.value.push(ubicacion)
  }

  // Actualizar estado de "seleccionar todas"
  seleccionarTodasUbicacionesQR.value = 
    ubicacionesQRSeleccionadas.value.length === ubicacionesFiltradasQR.value.length
}

function toggleSeleccionarTodasUbicacionesQR() {
  if (seleccionarTodasUbicacionesQR.value) {
    ubicacionesQRSeleccionadas.value = [...ubicacionesFiltradasQR.value]
  } else {
    ubicacionesQRSeleccionadas.value = []
  }
}

function agregarUbicacionesQRACola() {
  const nuevasUbicaciones = ubicacionesQRSeleccionadas.value.filter(ubicacion =>
    !colaUbicacionesQR.value.some(c => c.id === ubicacion.id)
  )

  colaUbicacionesQR.value.push(...nuevasUbicaciones)
  ubicacionesQRSeleccionadas.value = []
  seleccionarTodasUbicacionesQR.value = false
  
  mostrarNotificacion(`${nuevasUbicaciones.length} ubicaciones agregadas a la cola`, 'success')
}

function removerUbicacionQRDeCola(ubicacionId) {
  colaUbicacionesQR.value = colaUbicacionesQR.value.filter(ub => ub.id !== ubicacionId)
  mostrarNotificacion('Ubicación removida de la cola', 'info')
}
...
async function generarQRsParaCola() {
  ...
  // Generar QRs para ubicaciones
  for (const ubicacion of colaUbicacionesQR.value) {
    if (!ubicacion.qrDataUrl && ubicacion.codigo_qr) {
      ubicacion.qrDataUrl = await generarQRCode(ubicacion.codigo_qr)
    }
  }
