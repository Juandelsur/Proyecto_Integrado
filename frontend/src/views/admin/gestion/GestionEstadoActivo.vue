<template>
  <div class="crud-entidad-content">
    ...
    <div class="header-section">
      <h1 class="entity-title">Gestión Estados de Activos</h1>
      <v-btn
        color="blue"
        size="large"
        class="btn-nuevo"
    ...
    <div class="search-section">
      <v-text-field
        ...
        label="Buscar por nombre del estado"
        variant="outlined"
        density="comfortable"
        clearable
    ...
    <div v-if="loading" class="loading-container">
      ...
      <p class="mt-3">Cargando estados...</p>
    </div>
        ...
        <v-card-text class="pa-4">
          <v-form ref="formRef">
            ...
            <v-text-field
              ...
              autofocus
              hint="Ejemplo: Disponible, En uso, En reparación, etc."
              persistent-hint
            ></v-text-field>
          </v-form>
        </v-card-text>
    ...
    <v-dialog v-model="showDeleteDialog" max-width="400px">
      <v-card>
        ...
        <v-card-text class="pa-4">
          ...
          <br><br>
          <strong>{{ registroAEliminar?.nombre_estado }}</strong>
          <br>
          <span class="text-grey">ID: {{ registroAEliminar?.id }}</span>
          <br><br>
          <v-alert type="warning" density="compact" class="mt-2">
            Los activos que usen este estado quedarán sin estado asignado.
          </v-alert>
        </v-card-text>
...
/**
 * ============================================================================
 * GESTIÓN ESTADOS DE ACTIVOS - CRUD COMPLETO
 * ============================================================================
 *
 * Funcionalidades:
 * - Listar estados con paginación (20 por página)
 * - Búsqueda por nombre
 * - Crear nuevo estado
...
async function guardar() {
  ...
  try {
    ...
    if (modoEdicion.value) {
      ...
      await apiClient.put(
        `/api/estados-activo/${formulario.value.id}/`,
        payload
      )
      mostrarNotificacion('Estado actualizado correctamente', 'success')
    } else {
      // Crear nuevo estado
      await apiClient.post('/api/estados-activo/', payload)
      mostrarNotificacion('Estado creado correctamente', 'success')
    }
    ...
    if (error.response?.data) {
      ...
      let mensajeError = 'Error al guardar el estado'
...
async function eliminar() {
  try {
    await apiClient.delete(`/api/estados-activo/${registroAEliminar.value.id}/`)
    
    mostrarNotificacion('Estado eliminado correctamente', 'success')
    ...
    if (error.response?.status === 500 || error.response?.status === 400) {
      ...
      if (errorMsg && typeof errorMsg === 'string' && errorMsg.includes('ProtectedError')) {
        mostrarNotificacion(
          'No se puede eliminar este estado porque está siendo usado por uno o más activos', 
          'error'
        )
      } else {
        mostrarNotificacion('Error al eliminar el estado. Puede estar en uso por otros registros.', 'error')
      }
    } else {
      mostrarNotificacion('Error al eliminar el estado', 'error')
    }
