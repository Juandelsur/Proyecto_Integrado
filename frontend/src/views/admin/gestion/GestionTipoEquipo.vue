  ...
  <div class="crud-entidad-content">
    ...
    <div class="search-section">
      <v-text-field
        ...
        label="Buscar por nombre del Tipo de Equipo"
        variant="outlined"
        density="comfortable"
        clearable
    ...
    <div v-else class="registros-lista">
      ...
      <div v-if="registrosMostrados.length === 0" class="empty-state">
        ...
        <p>No hay Tipos de Equipo disponibles</p>
      </div>
    ...
    <v-dialog v-model="showModal" max-width="500px">
      <v-card>
        <v-card-title class="text-h5 pa-4 bg-primary text-white">
          ...
          {{ modoEdicion ? 'Editar tipo-equipo' : 'Nuevo tipo-equipo' }}
        </v-card-title>
    ...
    <v-dialog v-model="showDeleteDialog" max-width="400px">
      <v-card>
        ...
        <v-card-text class="pa-4">
          ¿Estás seguro de que deseas eliminar este Tipo de Equipo?
          <br><br>
          <strong>{{ registroAEliminar?.nombre_tipo }}</strong>
          <br>
          <span class="text-grey">ID: {{ registroAEliminar?.id }}</span>
          <br><br>
          <v-alert type="warning" density="compact" class="mt-2">
            Los activos que usen este Tipo de Equipo quedarán sin Tipo de Equipo asignado.
          </v-alert>
        </v-card-text>
...
/**
 * ============================================================================
 * GESTIÓN TIPO DE EQUIPO - CRUD COMPLETO
 * ============================================================================
 *
 * Funcionalidades:
 * - Listar Tipos de Equipo con paginación (20 por página)
 * - Búsqueda por nombre
...
async function cargarRegistros() {
  ...
  try {
    ...
    mostrarNotificacion('Error al cargar los Tipos de Equipo', 'error')
  } finally {
    loading.value = false
  }
}
...
/**
 * Guarda un nuevo Tipo de Equipo o actualiza uno existente
 */
async function guardar() {
  ...
  try {
    ...
    if (modoEdicion.value) {
      ...
      await apiClient.put(
        `/api/tipos-equipo/${formulario.value.id}/`,
        payload
      )
      mostrarNotificacion('Tipo de Equipo actualizado correctamente', 'success')
    } else {
      // Crear nuevo Tipo de Equipo
      await apiClient.post('/api/tipos-equipo/', payload)
      mostrarNotificacion('Tipo de Equipo creado correctamente', 'success')
    }
    ...
    if (error.response?.data) {
      ...
      let mensajeError = 'Error al guardar el Tipo de Equipo'
      ...
      mostrarNotificacion('Error al guardar el Tipo de Equipo', 'error')
    }
  }
}
...
async function eliminar() {
  try {
    await apiClient.delete(`/api/tipos-equipo/${registroAEliminar.value.id}/`)
    
    mostrarNotificacion('Tipo de Equipo eliminado correctamente', 'success')
    ...
    if (error.response?.status === 500 || error.response?.status === 400) {
      ...
      if (errorMsg && typeof errorMsg === 'string' && errorMsg.includes('ProtectedError')) {
        mostrarNotificacion(
          'No se puede eliminar este Tipo de Equipo porque está siendo usado por uno o más activos', 
          'error'
        )
      } else {
        mostrarNotificacion('Error al eliminar el Tipo de Equipo. Puede estar en uso por otros registros.', 'error')
      }
    } else {
      mostrarNotificacion('Error al eliminar el Tipo de Equipo', 'error')
    }
...
/**
 * Abre el modal para crear un nuevo Tipo de Equipo
 */
...
/**
 * Abre el modal para editar un Tipo de Equipo existente
 */
function abrirModalEditar(registro) {
  modoEdicion.value = true
  formulario.value = {
    id: registro.id,
    nombre_tipo: registro.nombre_tipo
  }
  showModal.value = true
}
