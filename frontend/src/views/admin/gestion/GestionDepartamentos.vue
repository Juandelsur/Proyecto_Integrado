<template>
  <div class="crud-entidad-content">
    ...
    <div class="header-section">
      <h1 class="entity-title">Gestión De Departamentos</h1>
      <v-btn
        color="blue"
        size="large"
        class="btn-nuevo"
    ...
    <div class="search-section">
      <v-text-field
        ...
        label="Buscar por nombre del Departamento"
        variant="outlined"
        density="comfortable"
        clearable
      ...
      <v-card>
        ...
        <v-card-text class="pa-4">
          <v-form ref="formRef">
            ...
            <v-text-field
              v-model="formulario.nombre_departamento"
              ...
              autofocus
              hint="Ejemplo: Urgencias, Laboratorio, Pediatria, etc."
              persistent-hint
            ></v-text-field>
          </v-form>
        </v-card-text>
    ...
    <v-dialog v-model="showDeleteDialog" max-width="400px">
      <v-card>
        ...
        <v-card-text class="pa-4">
          ...
          <br><br>
          <strong>{{ registroAEliminar?.nombre_departamento }}</strong>
          <br>
          <span class="text-grey">ID: {{ registroAEliminar?.id }}</span>
          <br><br>
          <v-alert type="warning" density="compact" class="mt-2">
            Los activos que usen este departamento quedarán sin departamento asignado.
          </v-alert>
        </v-card-text>
...
/**
 * ============================================================================
 * GESTIÓN DEPARTAMENTOS - CRUD COMPLETO
 * ============================================================================
 *
 * Funcionalidades:
 * - Listar departamentos con paginación (20 por página)
 * - Búsqueda por nombre
...
async function guardar() {
  ...
  try {
    const payload = {
      nombre_departamento: formulario.value.nombre_departamento?.trim()
    }
    ...
    if (modoEdicion.value) {
      ...
      await apiClient.put(
        `/api/departamentos/${formulario.value.id}/`,
        payload
      )
      mostrarNotificacion('Departamento actualizado correctamente', 'success')
    } else {
      // Crear nuevo departamento
      await apiClient.post('/api/departamentos/', payload)
      mostrarNotificacion('Departamento creado correctamente', 'success')
    }
    ...
    if (error.response?.data) {
      ...
      let mensajeError = 'Error al guardar el departamento'
